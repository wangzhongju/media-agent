                 ...
                 std::vector<object_result> *od_results)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (app_ctx->is_quant) {
            validCount += process_i8_rv1106((int8_t *)_outputs[box_idx]->virt_addr, app_ctx->output_attrs[box_idx].zp, app_ctx->output_attrs[box_idx].scale,
                                ...
                                class_num, grid_h, grid_w, stride, dfl_len, filterBoxes, objProbs, classId, conf_threshold);
        }
        else
        {
        ...
        validCount += process_u8((uint8_t *)_outputs[box_idx].buf, app_ctx->output_attrs[box_idx].zp, app_ctx->output_attrs[box_idx].scale,
                     ...
                     class_num, grid_h, grid_w, stride, dfl_len,
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        result.box.w = (int)(clamp(x2, 0, model_in_w) / letter_box->scale) - result.box.x;
        result.box.h = (int)(clamp(y2, 0, model_in_h) / letter_box->scale) - result.box.y;
        result.box.angle = 0;
