...
// box转化为角点
// 顺时针方向返回角点位置
static std::vector<float> rbbox_to_corners(const std::vector<float> &rbbox) {
...
// 检测点是否在四边形内的函数
static bool point_in_quadrilateral(float pt_x, float pt_y, const std::vector<float> &corners) {
...
// 检测线段相交并计算交点的函数
static int line_segment_intersection(const std::vector<float> &pts1, const std::vector<float> &pts2, int i, int j
...
// 比较函数，用于排序
static bool compare_points(const std::vector<float> &pt1, const std::vector<float> &pt2, const std::vector<float> &center) {
...
// 对凸多边形的顶点进行排序
static void sort_vertex_in_convex_polygon(std::vector<std::vector<float>> &int_pts, int num_of_inter) {
    if (num_of_inter > 0) {
...
// 计算三角形的面积
static float triangle_area(const std::vector<float> &a, const std::vector<float> &b, const std::vector<float> &c) {
    return std::abs((a[0] - c[0]) * (b[1] - c[1]) - (a[1] - c[1]) * (b[0] - c[0])) / 2.0;
}

// 计算多边形转化为多个三角形面积之和
static float polygon_area(const std::vector<std::vector<float>> &int_pts, int num_of_inter) {
    float area_val = 0.0;
...
static float Cal_IOU(float x1, float y1, float w1, float h1, float angle1, float x2, float y2, float w2, float h2, float angle2) {
    // 定义两个box的数据
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, int class_num, std::vector<object_result> *od_results) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (app_ctx->is_quant) {
            validCount += process_u8((uint8_t *)_outputs[i].buf, (uint8_t *)_outputs[3].buf, grid_h, grid_w, stride, filterBoxes, objProbs,
                                     classId, conf_threshold, class_num,
    ...
    for (int i = 0; i < validCount; ++i) {
        if (indexArray[i] == -1 || od_results->size() >= OBJ_NUMB_MAX_SIZE) {
            continue;
        }
        int n = indexArray[i];
        ...
        result.box.x = (int)(clamp(x1, 0, model_in_w) / letter_box->scale);
        result.box.y = (int)(clamp(y1, 0, model_in_h) / letter_box->scale);
        result.box.w = (int)(clamp(w, 0, model_in_w) / letter_box->scale);
        result.box.h = (int)(clamp(h, 0, model_in_h) / letter_box->scale);
        result.box.angle = angle;
        result.prop = obj_conf;
        result.class_id = id;
        result.class_name.clear();
        od_results->push_back(result);
    }
    return 0;
}
