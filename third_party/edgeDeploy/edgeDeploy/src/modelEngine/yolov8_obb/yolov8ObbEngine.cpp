// yolov8ObbEngine.cpp
// Merges yolov8_obb init/inference + OBB postprocess into a single C++ class.
// Uses RKNN zero-copy (ZERO_COPY path) for input/output tensor memory.

#include "yolov8_obb/yolov8ObbEngine.h"
#include "yolov8_obb/postprocess.h"
...
// ============================================================
Yolov8ObbEngine::Yolov8ObbEngine() {}

Yolov8ObbEngine::~Yolov8ObbEngine() {
    releaseResources();
}

void Yolov8ObbEngine::releaseResources() {
    if (!app_ctx_) return;
...
void Yolov8ObbEngine::dump_tensor_attr(rknn_tensor_attr *attr) {
    char dims[128] = {0};
...
//  infer
// ============================================================
int Yolov8ObbEngine::infer(const image_buffer_t &image,
...
int Yolov8ObbEngine::inferInternal(const image_buffer_t &image,
                           const filter_list_t &filters,
                           std::vector<object_result> &results,
                           image_buffer_t *drawed_image) {
    ...
    if (dst_img.virt_addr == NULL && dst_img.fd == 0) {
        std::cerr << "Yolov8ObbEngine::infer input mem invalid!" << std::endl;
        return -1;
    }
    ...
    if (ret < 0) {
        std::cerr << "Yolov8ObbEngine::infer letterbox failed ret=" << ret << std::endl;
        return -1;
    }
    ...
    for (uint32_t i = 0; i < n_output; ++i) {
        ...
        if (app_ctx_->is_quant) {
            ...
            std::cerr << "Yolov8ObbEngine: zero-copy does not support fp16 output" << std::endl;
    ...
    if (drawed_image != nullptr) {
        // ---- draw OBB boxes onto a copy of the original image AND normalize coordinates ----
...
void Yolov8ObbEngine::normalize_results(const image_buffer_t &image, std::vector<object_result> &results) {
    for (auto &obb : results) {
        obb.box.x = obb.box.x / (float)image.width;
        obb.box.y = obb.box.y / (float)image.height;
        obb.box.w = obb.box.w / (float)image.width;
        obb.box.h = obb.box.h / (float)image.height;
    }
}

void Yolov8ObbEngine::draw_and_normalize(const image_buffer_t &image, std::vector<object_result> &results, image_buffer_t &drawed_image) {
    ...
    if (drawed_image.virt_addr == nullptr) {
        std::cerr << "Yolov8ObbEngine::infer failed to alloc drawed_image" << std::endl;
        return;
    }
    ...
    for (auto &obb : results) {
        unsigned int color = CLASS_COLORS[obb.class_id % NUM_CLASS_COLORS];

        // 1. Draw OBB Box and Text via CPU (with FreeType in the future) onto the image
        // NOTE: The structure here reflects drawing rotated boxes and text via CPU.
        // It could then be overlaid via RGA `imblend` if doing multi-layer composite.
        draw_obb_rectangle(&drawed_image,
                           (int)obb.box.x, (int)obb.box.y,
                           (int)obb.box.w, (int)obb.box.h,
                           obb.box.angle, color, thickness);

        // label at the center of the OBB
        char label[128];
        if (!obb.class_name.empty()) {
            snprintf(label, sizeof(label), "%s %.2f", obb.class_name.c_str(), obb.prop);
        } else {
            snprintf(label, sizeof(label), "%d %.2f", obb.class_id, obb.prop);
        }
        draw_text(&drawed_image, label,
                  (int)obb.box.x, (int)obb.box.y > font_size ? (int)obb.box.y - font_size : 0,
                  color, font_size);

        // 2. Normalize coordinates: [xcenter, ycenter, w, h] to relative ratios
    }

    normalize_results(image, results);
}

int Yolov8ObbEngine::NC1HWC2_i8_to_NCHW_i8(const int8_t *src,
                                           int8_t *dst,
                                           int *dims,
                                           int channel,
                                           int h,
...
int Yolov8ObbEngine::post_process(rknn_output *_outputs, letterbox_t *letter_box,
                                   float conf_threshold, float nms_threshold,
                                   std::vector<object_result> &od_results) {
    int class_num = class_names_.empty() ? 0 : static_cast<int>(class_names_.size());
    int ret = yolov8_obb::post_process(app_ctx_, _outputs, letter_box, conf_threshold, nms_threshold, class_num, &od_results);
    if (ret != 0) {
        return ret;
    }
