...
static int convert_image_rga(image_buffer_t* src_img, image_buffer_t* dst_img, image_rect_t* src_box, image_rect_t* dst_box, char color)
{
    ...
    int srcWidthStride = src_img->width_stride > 0 ? src_img->width_stride : srcWidth;
    int srcHeightStride = src_img->height_stride > 0 ? src_img->height_stride : srcHeight;
    ...
    int dstWidthStride = dst_img->width_stride > 0 ? dst_img->width_stride : dstWidth;
    int dstHeightStride = dst_img->height_stride > 0 ? dst_img->height_stride : dstHeight;
    ...
    in_param.width = srcWidthStride;
    in_param.height = srcHeightStride;
    in_param.format = srcFmt;

    im_handle_param_t dst_param;
    dst_param.width = dstWidthStride;
    dst_param.height = dstHeightStride;
    ...
    if (use_handle) {
        ...
        rga_buf_src = wrapbuffer_handle(rga_handle_src, srcWidth, srcHeight, srcFmt, srcWidthStride, srcHeightStride);
    } else {
        if (src_phy != NULL) {
            rga_buf_src = wrapbuffer_physicaladdr(src_phy, srcWidth, srcHeight, srcFmt, srcWidthStride, srcHeightStride);
        } else if (src_fd > 0) {
            rga_buf_src = wrapbuffer_fd(src_fd, srcWidth, srcHeight, srcFmt, srcWidthStride, srcHeightStride);
        } else {
            rga_buf_src = wrapbuffer_virtualaddr(src, srcWidth, srcHeight, srcFmt, srcWidthStride, srcHeightStride);
        }
    }
    ...
    if (use_handle) {
        ...
        rga_buf_dst = wrapbuffer_handle(rga_handle_dst, dstWidth, dstHeight, dstFmt, dstWidthStride, dstHeightStride);
    } else {
        if (dst_phy != NULL) {
            rga_buf_dst = wrapbuffer_physicaladdr(dst_phy, dstWidth, dstHeight, dstFmt, dstWidthStride, dstHeightStride);
        } else if (dst_fd > 0) {
            rga_buf_dst = wrapbuffer_fd(dst_fd, dstWidth, dstHeight, dstFmt, dstWidthStride, dstHeightStride);
        } else {
            rga_buf_dst = wrapbuffer_virtualaddr(dst, dstWidth, dstHeight, dstFmt, dstWidthStride, dstHeightStride);
        }
    }
